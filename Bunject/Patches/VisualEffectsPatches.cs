...
{
	internal static class VisualEffectsControllerExtended
	{
		private static readonly HashSet<IModBunburrowStyleEffect> activeStyleEffects = new HashSet<IModBunburrowStyleEffect>();
		public static void AddStyleEffect(IModBunburrowStyleEffect styleEffect)
		{
			activeStyleEffects.Add(styleEffect);
		}
		public static void ClearStyleEffects()
		{
			activeStyleEffects.Clear();
		}
		public static void SetStyleParticlesActive(bool on)
		{
			foreach (var x in activeStyleEffects)
				x.SetStyleParticleActive(on);
		}
		public static void SetStyleEffectsActive(bool on)
		{
			foreach (var x in activeStyleEffects)
				x.SetStyleVisualActive(on);
		}
	...
	internal class UpdateVoidVisualEffectsPatch
	{
		private static bool Prefix(bool __runOriginal) //, VisualEffectsController __instance)
		{
			...
			{
				if (level.StyleEffects.Contains(StyleEffectsManager.Void))
				{
					StyleEffectsManager.Void.SetStyleParticleActive(SettingsManager.HasParticles);
					StyleEffectsManager.Void.SetStyleVisualActive(SettingsManager.HasVisualEffects);
				}
				/*
				if (level.StyleEffects.Contains(AssetsManager.BunburrowsListOfStyles.VoidB) && SettingsManager.HasVisualEffects)
				{
  ...
  internal class UpdateVisualEffectsPatch
	{
		...
		{
			VisualEffectsControllerExtended.SetStyleParticlesActive(false);
			VisualEffectsControllerExtended.SetStyleEffectsActive(false);
			VisualEffectsControllerExtended.ClearStyleEffects();
		}
    [HarmonyPrefix, HarmonyPriority(Priority.Normal)]
    private static bool Replace(bool __runOriginal, VisualEffectsController __instance)
    {
			if (__runOriginal && GameManager.CurrentLevel.BaseData is ModLevelObject level)
			{

				foreach(var eff in level.StyleEffects)
				{
					VisualEffectsControllerExtended.AddStyleEffect(eff);
				}
				VisualEffectsControllerExtended.SetStyleEffectsActive(SettingsManager.HasVisualEffects);
				VisualEffectsControllerExtended.SetStyleParticlesActive(SettingsManager.HasParticles);
				return false;
			}
			return true;
		}
		...
		if (__runOriginal && GameManager.CurrentLevel.BaseData is ModLevelObject level)
		{
			var effects = level.StyleEffects;
